...
import { CheckCircle, AlertTriangle, XCircle, Loader2, X, ArrowRight, Plus, FileText, Image as ImageIcon } from "lucide-react";
...
const HARDCODED_ANALYSIS = [
  {
    ...
    tag: <span className="bg-red-100 text-red-700 px-2 py-0.5 rounded text-xs ml-2">LOW</span>,
  ...
  },
  {
    ...
    tag: <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded text-xs ml-2">NORMAL</span>,
  ...
  },
  {
    ...
    tag: <span className="bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded text-xs ml-2">BORDERLINE</span>,
...
function getFileTypeIcon(type: string) {
  if (type.startsWith("image/")) return <ImageIcon className="w-4 h-4 text-blue-500 mr-1" />;
  if (type === "application/pdf") return <FileText className="w-4 h-4 text-red-500 mr-1" />;
...
export default function AIReportAnalyzer() {
  ...
  useEffect(() => {
    if (files.length > 0) {
      ...
      setTimeout(() => {
        ...
        result += "\n**What This Means:**" + HARDCODED_SUMMARY + "\n\n**Next Steps:**\n" + HARDCODED_NEXT_STEPS.map(s => `- ${s}`).join("\n");
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const fileObjs = validFiles.map(file => ({ file, url: file.type.startsWith("image/") ? URL.createObjectURL(file) : "" }));
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    const validFiles = dtFiles.filter(f => f.type.startsWith("image/") || f.type === "application/pdf");
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white">
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Analyze a Medical Report</h1>
          <p className="text-lg text-gray-600">Upload your lab reports (PDF, JPG, PNG) and let AI explain them in simple language.</p>
        </div>
        <div className="flex flex-col lg:flex-row gap-8">
          {/* Left Panel: Drop Zone */}
          <Card
            className={`flex-1 flex flex-col items-center justify-center min-h-[400px] border-2 border-dashed transition-colors ${dragActive ? "border-blue-400 bg-blue-50" : "border-slate-300 bg-white dark:bg-slate-900"}`}
            onDrop={handleDrop}
          ...
          >
            <CardHeader className="flex flex-col items-center gap-1 mb-2">
              <CardTitle className="text-lg font-bold text-center">Lab Report</CardTitle>
              <CardDescription className="text-base text-slate-600 text-center">Drag & drop your reports here, or click to upload. (PDF, JPG, PNG)</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col items-center w-full">
              ...
              <div className="flex flex-wrap gap-2 w-full mb-4">
                {files.map((f, idx) => (
                  <div
                    key={idx}
                    className={`flex items-center px-2 py-1 rounded border ${selectedIdx === idx ? "border-blue-500 bg-blue-50" : "border-slate-200 bg-slate-100"} cursor-pointer`}
                    onClick={e => { e.stopPropagation(); setSelectedIdx(idx); }}
                  >
                    {getFileTypeIcon(f.file.type)}
                    <span className="text-xs font-medium mr-1">{f.file.name}</span>
                    <button onClick={e => { e.stopPropagation(); handleRemoveFile(idx); }} className="ml-1 text-red-500 hover:text-red-700" aria-label="Remove file"><X size={14} /></button>
                  </div>
                ))}
              </div>
              {files[selectedIdx] ? (
                files[selectedIdx].file.type.startsWith("image/") ? (
                  ...
                  <div className="flex flex-col items-center mt-8">
                    ...
                    <span className="text-sm text-slate-700">{files[selectedIdx].file.name}</span>
                    <span className="text-xs text-slate-500">PDF file (preview not available)</span>
                  </div>
                )
              ) : (
                <img
            ...
            <CardContent>
              {isAnalyzing ? (
                <div className="flex flex-col items-center justify-center h-full py-16">
                  ...
                  <p className="text-blue-700 font-medium">AI is analyzing your report...</p>
                </div>
              ) : typedAnalysis ? (
                <div>
                  ...
                  <ul className="mb-4">
                    {HARDCODED_ANALYSIS.map((item, idx) => (
                      <li key={idx} className="flex items-center gap-2 mb-1">
                        ...
                        {item.tag}
                        <span className="ml-2 text-xs text-slate-500">{item.explanation}</span>
                      </li>
                    ))}
                  </ul>
                  <h4 className="font-semibold mb-2">What This Means:</h4>
                  <div className="mb-4 whitespace-pre-line text-slate-700">{HARDCODED_SUMMARY}</div>
                  ...
                  <div className="flex gap-4 mt-6">
                    <Button variant="default" className="flex-1"><Plus className="w-4 h-4 mr-1" />Save to My Medical Records</Button>
                    <Button variant="outline" className="flex-1" onClick={() => window.location.href='/doctor-directory'}><ArrowRight className="w-4 h-4 mr-1" />Find a Specialist</Button>
                  </div>
                </div>
              ) : (
                <div className="text-gray-400 text-center mt-20">Upload a report to see the analysis here.</div>
              )}
            </CardContent>
          </Card>
        </div>
